...
use App\Topnode\BaseBundle\Doctrine\EntityClassGenerator;
use App\Topnode\BaseBundle\Utils\SourceManipulation\ClassSourceManipulator;
...
final class MakeTopEntity extends AbstractMaker implements InputAwareMakerInterface
{
    private $fileManager;
    ...
    public function configureCommand(Command $command, InputConfiguration $inputConf)
    {
        $command
            ...
            ->addOption('not-a-crud', null, InputOption::VALUE_NONE, 'Not add Crud attributes to entity')
            ->addOption('identifier', null, InputOption::VALUE_NONE, 'Not add identifier field on entity')
        ;
    ...
    public function interact(InputInterface $input, ConsoleStyle $io, Command $command)
    {
        ...
        $question->setValidator(function ($anws) {
            preg_match('/(Type)|(type)/', $anws, $ilegalMatches);

            if (!empty($ilegalMatches)) {
                throw new \Exception('The class name cannot contains "type" string variations');
            }
    ...
    private function askForNextField(ConsoleStyle $io, array $fields, string $entityClass, bool $isFirstField)
    {
        ...
        $fieldName = $io->ask($questionText, null, function ($name) use ($fields) {
            ...
            $name = lcfirst(Str::asCamelCase($name));
    ...
    private function askRelationDetails(ConsoleStyle $io, string $generatedEntityClass, string $type, string $newFieldName)
    {
        ...
        while (null === $targetEntityClass) {
            ...
            $answeredEntityClass = ucfirst($answeredEntityClass);
            ...
            if ($this->fileManager->fileExists($targetEntityClass)) {
                $io->error(sprintf('Unknown class "%s"', $answeredEntityClass));
                $targetEntityClass = null;
                continue;
            }
